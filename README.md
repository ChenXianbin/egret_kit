...
## 套件示例使用
   ...
   部分常用接口调用示例，可见dev_kit/src/main.ts 内 createGameScene()方法，注意部分示例需要配置好服务器地址方可使用
    
## 分包功能介绍：

   套件内参考  [如何利用微信小游戏的分包加载机制突破4M代码包体积限制](https://mp.weixin.qq.com/s/9vg2cA1L-_eY2lTdbTSVeQ "如何利用微信小游戏的分包加载机制突破4M代码包体积限制")  文章内提供的分包示例并进行改良,基础用法与文章介绍相同：
   
   1.完善JS代码分包生成机制，第三方库亦可引进分包内，优化分包检测代码语句算法
   
   2.提供resource文件夹分包引用示例，资源包与主包轻松分离
   
   ![subpackage配置示例](https://github.com/ChenXianbin/egret_kit/blob/master/subpackage_1.png)
   
   3.提供loading界面引用主包图片资源进行绘制的示例，再也不用看着枯燥的数字了
   
   ![loading界面示例](https://github.com/ChenXianbin/egret_kit/blob/master/subpackage_2.jpg)
